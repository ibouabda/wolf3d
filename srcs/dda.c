...
/*   Updated: 2020/01/22 16:56:37 by ibouabda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_dda(t_env *e)
{
    ...
    || (e->rayang >= 0.0 && e->rayang <= 90.0))
	{
		e->playerx.x = 1 - (e->player.x - ((int)(e->player.x)));
		if ((int)e->playerx.x == 1)
		{
			e->playerx.y = e->player.y;
			e->playerx.x = e->playerx.x;
		}
		else
		{
	...
	{
		e->playery.y = 1 - (e->player.y - ((int)(e->player.y)));
		if ((int)e->playery.y == 1)
		{
			e->playery.x = e->player.x;
			e->playery.y = e->player.y;
		}
